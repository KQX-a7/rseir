...
parse_observed_data_for_score_calculation <- function(state_ac)
{
  ...
  ## import the all observed data
  {
    ...
    if ("can_US" %in% c(case_datasource, hosp_datasource, death_datasource)) {
      can_US_raw_timeseries <-
        read_csv(glue("{observed_dataset_folder}/can_US_dataset/US_timeseries.csv"), col_types = "cDcccciiiiiiiiiiiiii")
    ...
    if ("esri_CA" %in% c(case_datasource, hosp_datasource, death_datasource)) {
      esri_CA_raw_timeseries <-
        read_csv(glue("{observed_dataset_folder}/esri_CA_dataset/CA_timeseries.csv"), col_types = "icciciiiiiiiiiiiii")
...
make_plots <- function(state_ac)
{
  ...
  temp_plot <- ggplot() +
    geom_line(aes(y = SEIRD_pred$cumulative_I1*test_fraction, x = SEIRD_pred$date),
              color = "red") +
    geom_point(aes(y = target_data$case$cumulative_cases, x = target_data$case$date), color = "blue") +
    labs(
      x = "Date",
      y = "cumulative_I1",
  ...
  temp_plot <- ggplot() +
    geom_line(aes(y = SEIRD_pred$D,
                  x = SEIRD_pred$date),
              color = "red") +
    geom_point(aes(y = target_data$death$cumulative_deaths, x = target_data$death$date),
               color = "blue") +
    labs(
      x = "Date",
      y = "cumulative_D",
...
write_csv(state_info,glue("inputs/{country}_state_info.csv"))
```
...
## target setup
# for "US", data_source "can" and "f_1m3f" is available.
# for "CA", data_source "c19ca" and "f_1m3f" is available.
country <- "US"  # "US" or "CA"
...
state <-  c("AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY") # c("IL","NY", "FL", "CA") #c("IL","NY", "FL", "CA") # c("CA")
