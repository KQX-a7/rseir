...
parse_observed_data_for_score_calculation <- function(state_ac)
{
  ...
  my_smooth <- function(vec,radius){
    ...
    res_vec <- (lag(vec,n=radius)+vec+lead(vec,n=radius))/3
    return(res_vec)
  }
  target_timeseries <- target_timeseries %>% 
    mutate(across(!c(state,days_to_ref), ~ my_smooth(vec = .x,radius = window_radius)))
  head(target_timeseries)
  ##
  ...
  with(target_timeseries,
       {
         ...
         I1_std <- rep(1,length(I1_std))
         D_std <- rep(1,length(D_std))
         hosp_std <- rep(1,length(hosp_std))
         ##
...
## target setup
country <- "US"  # "US" or "CA"
data_source <- "can" # "can" or "1m3f"
observed_dataset_path <- glue("inputs/{data_source}_dataset/{country}_timeseries.csv")
...
use_cases <- T
use_hosp <- T   # hosp data is not available for 1m3f dataset, please set as F if using 1m3f data.
use_death <- F 
