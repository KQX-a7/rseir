...
rpyseir <- function()
{
  ...
  for (state_ac in state) 
  {
    ...
    if (overwrite_initial_dd_parameter == T) {
      ...
      # initial_dd_parameter[5] <-
      #   as.integer(sampling_para$lockdown_date - reference_date)
      # dd_parameter_upper[5] <-
      #   initial_dd_parameter[5] + opt_radius_of_policydate
      # dd_parameter_lower[5] <-
      #   initial_dd_parameter[5] - opt_radius_of_policydate
      # 
      # initial_dd_parameter[7] <-
      #   as.integer(sampling_para$reopen_date - reference_date)
      # dd_parameter_upper[7] <-
      #   initial_dd_parameter[7] + opt_radius_of_policydate
      # dd_parameter_lower[7] <-
      #   initial_dd_parameter[7] - opt_radius_of_policydate
      ###
      
      ### 20201130 pin date update
      initial_dd_parameter[5] <-
        as.integer(sampling_para$p1_to_p2_date - reference_date)
      dd_parameter_upper[5] <-
        initial_dd_parameter[5] + opt_radius_of_policydate
      dd_parameter_lower[5] <-
        initial_dd_parameter[5] - opt_radius_of_policydate
      ...
      initial_dd_parameter[7] <-
        as.integer(sampling_para$p2_to_p3_date - reference_date)
      dd_parameter_upper[7] <-
        initial_dd_parameter[7] + opt_radius_of_policydate
      dd_parameter_lower[7] <-
        initial_dd_parameter[7] - opt_radius_of_policydate
      
      initial_dd_parameter[9] <-
        as.integer(sampling_para$p3_to_p4_date - reference_date)
      dd_parameter_upper[9] <-
        initial_dd_parameter[9] + opt_radius_of_policydate
      dd_parameter_lower[9] <-
        initial_dd_parameter[9] - opt_radius_of_policydate
    ...
    make_plots(state_ac)
    print(glue("plot create at {Sys.time()}"))
    print("**************")
...
parse_observed_data_for_score_calculation <- function(state_ac)
{
  ...
  observed_dataset_path <-
    glue("inputs/observed_timerseries_dataset/{data_source}_dataset/{country}_timeseries.csv")
  target_data_rds_path <-
    glue("outputs/{country}/temp_rds/{country}_{state_ac}_target_data.rds")
  
  ## smooth the observed increase/current data 2020/10/21
  ...
  if(data_source %in% c("can","c19ca") )
  {
    ## different observed data import process for US and CA
    if (data_source == "can" && country == "US")
    {
      raw_timeseries <-
        read_csv(observed_dataset_path, col_types = "cDcccciiiiiiiiiiiiii")
      ...
      ## the saved target_data should have state, date, days_to_ref, cumu and increase
    ...
    else if (data_source == "c19ca" && country == "CA")
    {
      raw_timeseries <-
        read_csv(observed_dataset_path, col_types = "icciciiiiiiiiiiiii")
  ...
  else if (data_source == "1m3f") {
    raw_timeseries <- read_csv(observed_dataset_path)
    target_timeseries <- raw_timeseries %>%
      filter(
        state_name == state_ac,
...
make_plots <- function(state_ac)
{
  ...
  target_data <- 
    readRDS(file = glue("outputs/{country}/temp_rds/{country}_{state_ac}_target_data.rds"))
  ...
  temp_plot <- ggplot() +
    geom_line(aes(y = SEIRD_pred$cumulative_I1*test_fraction, x = SEIRD_pred$date),
              color = "red") +
    geom_point(aes(y = target_data$cumulative_cases, x = target_data$date), color = "blue") +
    labs(
      x = "Date",
      y = "cumulative_I1",
  ...
  if (data_source %in% c("can", "c19ca"))
  {
    ...
    if (obs_data_kit$hosp_flag == "cumulative")
    {
      temp_plot <- ggplot() +
        ...
        color = "red") +
        geom_point(aes(y = target_data$cumulative_hospitalized, x = target_data$date),
                   color = "blue") +
        labs(
          x = "Date",
  ...
  temp_plot <- ggplot() +
    geom_line(aes(y = SEIRD_pred$D,
                  x = SEIRD_pred$date),
              color = "red") +
    geom_point(aes(y = target_data$cumulative_deaths, x = target_data$date),
               color = "blue") +
    labs(
      x = "Date",
      y = "cumulative_D",
  ...
  temp_plot <- ggplot() +
    geom_line(aes(x = SEIRD_pred$date, y = build_beta1t(supt_para, s_para$delta)), color =
                "red") +
    labs(
      x = "Date",
      y = "beta1t",
...
folder_list <- c("outputs/CA/SEIRD_data","outputs/CA/plots_by_state","outputs/CA/temp_rds","outputs/US/SEIRD_data","outputs/US/plots_by_state","outputs/US/temp_rds")
...
dd_to_supt <- function(dd_para)
{
  ...
  if (number_of_phase >= 4)
  {
    ...
    if (supt_para[4] < 0 || supt_para[6] < 0)
    {
      print("warning: negaitive duration")
    }
  }
...
## target setup
# for "US", data_source "can" and "1m3f" is available.
# for "CA", data_source "c19ca" and "1m3f" is available.
country <- "US"  # "US" or "CA"
data_source <- "1m3f" # "can", "1m3f" or "c19ca"
...
state <- c("AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY") #c("IL","NY", "FL", "CA") # c("CA")
...
## the observed SEIRD variables that are used in the score calculation
use_case <- T
use_hosp <- F   # hosp data is not available for 1m3f dataset, please set as F if using 1m3f data.
use_death <- T 

## for data-driven parameter
# use date (pin-policy update) or use duration?
overwrite_initial_dd_parameter <- T
# the number of time phase. Now with pinned policy it must be 3.
number_of_phase <- 4
# the optimization radius of the policy date
opt_radius_of_policydate <- 30
...
dd_parameter_upper <- c(0.5,1,80,4.5,60,1.2,100,2,120,2)
dd_parameter_lower <- c(0.02,0.25,10,2,5,0.2,14,0.1,14,0.1)
...
## to add phases: change setup_parameters, build_beta1t, vline function in make plot
```
