...
rpyseir <- function()
{
  ...
  check_for_update_1m3f(force_update,skip_update)
  ...
  for (state_ac in state) 
  {
    ...
    if(need_replace==T){
      ...
      if(!dir.exists(sftb_plot_folder)){
        dir.create(sftb_plot_folder,recursive = T)
      }
      ...
      if (!dir.exists(glue("so_far_the_best/{country}/SEIRD_data"))) {
        dir.create(glue("so_far_the_best/{country}/SEIRD_data"),recursive = T)
      }
      file.copy(
      ...
      if (!dir.exists(sftb_rds_folder)) {
        dir.create(sftb_rds_folder,recursive = T)
      }
      rds_file_list <-
...
check_for_update_1m3f <- function(force=F,skip=F)
{
  ...
  if (force == T || skip != T && time_diff > 48) {
    ...
    file.copy(from = "inputs/observed_timerseries_dataset/f_1m3f_dataset/CA_timeseries.csv",
              to = "inputs/observed_timerseries_dataset/f_1m3f_dataset/out_of_date",
              overwrite = T)
    download.file(url = "https://instant.1point3acres.com/v1/api/coronavirus/us/cases?token=Dljs8daW", destfile = "inputs/observed_timerseries_dataset/f_1m3f_dataset/US_timeseries.csv")
    download.file(url = "https://instant.1point3acres.com/v1/api/coronavirus/canada/cases?token=Dljs8daW", destfile = "inputs/observed_timerseries_dataset/f_1m3f_dataset/CA_timeseries.csv")
