...
6.The observed dataset is "inputs/CA_20200919_Provincial_Daily_Totals.csv", from https://resources-covid19canada.hub.arcgis.com/datasets/provincial-daily-totals. To update it, download the csv file, rename it and place it on the given path.
...
rpyseir <- function()
{
  ...
  for (state_ac in state)
  {
    ...
    saveRDS(opt_res, 
            file = paste0("outputs/temp_rds/", state_ac, "_opt_res.rds"))
    saveRDS(sampling_para,
            file = paste0("outputs/temp_rds/", state_ac, "_sampling_para.rds"))
    saveRDS(observed_data_kit,
            file = paste0("outputs/temp_rds/", state_ac, "_observed_data_kit.rds"))
    SEIRD_pred <-
    ...
    write_csv(SEIRD_pred,
              path = paste0("outputs/SEIRD_data/", state_ac, "_SEIRD_pred.csv"))
    saveRDS(SEIRD_pred,
            file = paste0("outputs/temp_rds/", state_ac, "_SEIRD_pred.rds"))
...
{
  raw_timeseries <- read_csv("inputs/CA_20200919_Provincial_Daily_Totals.csv",
  ...
  saveRDS(target_timeseries,file = paste0("outputs/temp_rds/", state_ac, "_target_obs.rds"))
  ...
  with(target_timeseries,
       {
         ...
         ### notice: the usage of cumulative is disabled
         ### since canada don't have cumulative hosipital observed data
         ### hosp_flag is set to "current"
         ### so none of the operation about "cumulative" will be execute later
...
score_function <- function(dd_para, s_para, obs_data_kit)
{
  ...
  with(obs_data_kit, {
    ...
    #######
    if(length((1:total_days) + t0)!=length(I1_increase*test_fraction))
    {
      print(length(I1_increase*test_fraction))
      saveRDS(SEIRD_pred,file = "cache.rds")
      saveRDS(dd_para,file="cache2.rds")
      saveRDS(param_set)
    }
    #######
    ...
    ###
    # print(D_std)
    # cat("pred ",any(is.na(D_pred)),"\n")
    # cat("obs ",any(is.na(D_obs)),"\n")
    # cat("std ",any(is.na(D_std)),"\n")
    # cat("chi2_I1 = ",chi2_I1)
    # cat("chi2_D = ",chi2_D)
    # cat("chi2_hosp = ",chi2_hosp)
    ###
...
make_plots <- function(state_ac)
{
  ...
  dd_para <-
    readRDS(file = paste0("outputs/temp_rds/", state_ac, "_opt_res.rds"))$par
  opt_res <- 
    readRDS(file = paste0("outputs/temp_rds/", state_ac, "_opt_res.rds"))
  s_para <-
    readRDS(file = paste0("outputs/temp_rds/", state_ac, "_sampling_para.rds"))
  obs_data_kit <-
    readRDS(file = paste0("outputs/temp_rds/", state_ac, "_observed_data_kit.rds"))
  SEIRD_pred <-
    readRDS(file = paste0("outputs/temp_rds/", state_ac, "_SEIRD_pred.rds"))
  target_obs <- readRDS(file = paste0("outputs/temp_rds/", state_ac, "_target_obs.rds"))
  ...
  ggsave(paste0(state_ac, "_SEIRD_variables.png"),
         plot = temp_plot,
         path = "outputs/plots/all_variables")
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  ggsave(paste0(state_ac, "_cumulative_I1.png"),
         plot = temp_plot,
         path = "outputs/plots/cumulative_I1/obs_and_pred")
  ...
  ggsave(
    paste0(state_ac, "_cumulative_I1_filtered.png"),
    plot = temp_plot,
    path = "outputs/plots/cumulative_I1/obs_follow_by_pred"
  )
  ...
  if (obs_data_kit$hosp_flag == "cumulative")
  {
    ...
    temp_plot <- Add_timestamp_vline(temp_plot)
    ggsave(
      paste0(state_ac, "_cumulative_I2_plus_I3.png"),
      plot = temp_plot,
      path = "outputs/plots/cumulative_I2_plus_I3/obs_and_pred"
    )
  }
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  ggsave(paste0(state_ac, "_cumulative_D.png"),
         plot = temp_plot,
         path = "outputs/plots/cumulative_D/obs_and_pred")
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  ggsave(paste0(state_ac, "_I1_increase.png"),
         plot = temp_plot,
         path = "outputs/plots/I1_increase/obs_and_pred")
  ...
  ggsave(paste0(state_ac, "_I1_increase_zoomed.png"),
         plot = temp_plot,
         path = "outputs/plots/I1_increase/obs_follow_by_pred")
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  temp_plot
  ggsave(paste0(state_ac, "_I1_increase_ofbp.png"),
         plot = temp_plot,
         path = "outputs/plots/I1_increase/obs_follow_by_pred")
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  ggsave(paste0(state_ac, "_I2_plus_I3_increase.png"),
         plot = temp_plot,
         path = "outputs/plots/I2_plus_I3_increase/obs_and_pred")
  ...
  ggsave(paste0(state_ac, "_I2_plus_I3_increase_zoomed.png"),
         plot = temp_plot,
         path = "outputs/plots/I2_plus_I3_increase/obs_follow_by_pred")
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  ggsave(paste0(state_ac, "_I2_plus_I3_increase_ofbp.png"),
         plot = temp_plot,
         path = "outputs/plots/I2_plus_I3_increase/obs_follow_by_pred")
  }
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  ggsave(paste0(state_ac, "_D_increase.png"),
         plot = temp_plot,
         path = "outputs/plots/D_increase/obs_and_pred")
  ...
  ggsave(paste0(state_ac, "_D_increase_zoomed.png"),
         plot = temp_plot,
         path = "outputs/plots/D_increase/obs_follow_by_pred")
  ...
  temp_plot <- Add_timestamp_vline(temp_plot)
  ggsave(paste0(state_ac, "_D_increase_ofbp.png"),
         plot = temp_plot,
         path = "outputs/plots/D_increase/obs_follow_by_pred")
  ...
  for (v_name in c("S", "E", "I1", "I2", "I3", "R", "D"))
  {
    ...
    ggsave(filename = paste0(state_ac,"_",v_name,"_current.png"),
           plot = temp_plot,
           path = paste0("outputs/plots/all_variables_one_by_one/",v_name,"_current"))
  }
  ...
  if(obs_data_kit$hosp_flag=="current")
  {
    ...
    temp_plot <- Add_timestamp_vline(temp_plot)
    ggsave(
      filename = paste0(state_ac, "_I2_plus_I3_current.png"),
      plot = temp_plot,
      path = "outputs/plots/current_I2_plus_I3"
    )
  }
  ...
  ggsave(filename = paste0(state_ac,"_beta1t_and_dd_para.png"),
         plot = temp_plot,
         path = "outputs/plots/beta1t")
...
# the runtime is about 5min for IL
# the follow states are not available : NT, NU, PE, RC, YT
# the reason is lack of data. Their cumulaitve I1 is less than 60.
state <- c("AB","BC","CA","MB","NB","NL","NS","ON","QC","SK")
# please use obj state to run for all state, instead of changing all_state
# setting all_states as TRUE will also run for the lack-of-data states.
all_states <- FALSE

## basic setup
# state_info <- get_state_info()
state_info <- read_csv("inputs/CA_state_info.csv")
...
dd_parameter_lower <- c(0.02,0.25,10,2,5,0.2,14,0.2)
